...
from mycode.reques_to_java import *
import sys
...
class VideoTracker(object):
    ...
    def __enter__(self):
        ...
        if self.args.save_path:
            ...
            self.save_video_path = os.path.join(self.args.save_path, "circle25_results.mp4")
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                # calculate center landmark of car to custom line, line(x1,y1, x2, y2) tow point
                car_count = calculate_distance(bbox_xyxy, identities, line=(0, 200, 200, 200))

                img0 = draw_boxes(img0, bbox_xyxy, identities, str(car_count), line=(0, 200, 200, 200))  # BGR
            ...
            if self.args.save_img:
                cv2.imwrite(self.args.save_img+f'/{idx_frame}.jpg',img0)

            if self.args.save_txt:
                # with open(self.args.save_txt + str(idx_frame).zfill(4) + '.txt', 'w') as f:
                #     for i in range(len(outputs)):
                #         x1, y1, x2, y2, idx = outputs[i]
                #         f.write('{}\t{}\t{}\t{}\t{}\n'.format(x1, y1, x2, y2, idx))
                for i in range(len(outputs)):
                    x1, y1, x2, y2, idx = outputs[i]
                    r_f.write('{},{},{},{},{},{}\n'.format(idx_frame,x1, y1, x2, y2, idx))
                pass

            # todo 每5帧存一下
            # if self.args.asy_http:
            #     if idx_frame % 5 == 0:
            #         # to_java(outputs=outputs.insert(0,idx_frame))
            #         for i in range(len(outputs)):
            #             x1, y1, x2, y2, idx = outputs[i]
            #             info_list.append([x1, y1, x2, y2, idx, idx_frame])
            #
            #         pass
...
if __name__ == '__main__':
    ...
    # input and output
    parser.add_argument('--input_path', type=str, default='./testDir/circle25.mp4', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--save_path', type=str, default='output', help='output folder')  # output folder
    parser.add_argument('--save_img', type=str, default=False, help='output folder')
    parser.add_argument("--frame_interval", type=int, default=1)
    parser.add_argument('--fourcc', type=str, default='mp4v', help='output video codec (verify ffmpeg support)')
    parser.add_argument('--device', default='cpu', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--save_txt', default=False, help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--asy_http', default=False, help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=[2,5,7], help='filter by class')
    ...
    #todo request
    # th1 = MyThread(to_java,info_list)
    # if len(info_list) > 3:
    #     th1.run()
