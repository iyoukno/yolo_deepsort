...
@File    ：make_deepsort_datasets.py
@Author  ：yuk
@Date    ：2023/11/1 10:02 
description：
'''
...
PATH_DATASET = r'D:\datasets\car_reid\crop_cars_imgs'  # 需要处理的文件夹
PATH_NEW_DATASET = 'D:\datasets\car_reid\crop_cars_new'  # 处理后的文件夹
        ...
        for aroot, dirs, files in os.walk(self.path):
            ...
            for item in filelist:
                if item.endswith('.jpg'):  # 初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可）
                    ...
                    # 修改图片尺寸到128宽*256高
                    im = Image.open(src)
                    out = im.resize((64, 128), Image.LANCZOS)  # resize image with high-quality
                    out.save(src)  # 原路径保存
